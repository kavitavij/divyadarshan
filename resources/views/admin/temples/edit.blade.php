...
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{ route('admin.temples.update', $temple->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="mb-3"><label for="name" class="form-label">Name</label><input type="text"
                            name="name" class="form-control" id="name" value="{{ $temple->name }}"></div>
                    <div class="mb-3"><label for="location" class="form-label">Location</label><input type="text"
                            name="location" class="form-control" id="location" value="{{ $temple->location }}"></div>
                    <div class="mb-3"><label for="description" class="form-label">Short Description</label>
                        <textarea class="form-control" style="height:100px" name="description" id="description">{{ $temple->description }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Page Content (with HTML/CSS Support)</h3>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                                ...
                                <div class="card-body">
                                    @foreach ($calendar['days'] as $day)
                                        <div class="mb-2 d-flex align-items-center">
                                            ...
                                            <select name="slot_data[{{ $day['date'] }}]" id="slot_{{ $day['date'] }}"
                                                class="form-select form-select-sm">
                                                <option value="not_available"
                                                    @if ($day['status'] == 'not_available') selected @endif>Not Available</option>
                                                <option value="available" @if ($day['status'] == 'available') selected @endif>
                                                    Available</option>
                                                <option value="full" @if ($day['status'] == 'full') selected @endif>
                                                    Full</option>
                                            </select>
                                        </div>
                                    @endforeach
            ...
            <div class="mb-3">
                ...
                <div>
                    @if ($temple->image)
                        <img src="{{ asset($temple->image) }}" height="100" class="mb-2"
                            alt="{{ $temple->name }}">
                    @endif
                </div>
