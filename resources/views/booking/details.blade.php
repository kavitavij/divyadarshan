    ...
    <div class="container py-5">
        <div class="progress-stepper">
            ...
            <div class="step active">
                ...
                <div class="step-label">Add Devotee(s) Details</div>
            </div>
            <div class="step">
                    ...
                    <div class="card-body p-4">
                        @if ($errors->any())
                            <div class="alert alert-danger mb-4">
                                <h5 class="alert-heading">Please fill the correct information below:</h5>
                                <ul class="mb-0">
                        ...
                        <form id="devotee-form" action="{{ route('cart.addDarshan') }}" method="POST"
                            ...
                            <input type="hidden" name="darshan_slot_id"
                                value="{{ $bookingData['darshan_slot_id'] ?? '' }}">
                                ...
                                @for ($i = 0; $i < ($bookingData['number_of_people'] ?? 0); $i++)
                                    <div class="accordion-item">
                                        ...
                                        <div id="collapse{{ $i }}"
                                            ...
                                            data-bs-parent="#devoteeAccordion">
                                            <div class="accordion-body">
                                                <div class="row g-3">
                                                    <div class="col-12"><label class="form-label">Full
                                                            Name</label><input type="text"
                                                            name="devotees[{{ $i }}][full_name]"
                                                            class="form-control devotee-input"
                                                            data-index="{{ $i }}" data-field="full_name"
                                                            required></div>
                                                    <div class="col-md-6"><label class="form-label">Gender</label>
                                                        <div class="d-flex">
                                                            <div class="form-check me-3"><input class="form-check-input"
                                                                    type="radio"
                                                                    name="devotees[{{ $i }}][gender]"
                                                                    id="male_{{ $i }}" value="male"
                                                                    checked><label class="form-check-label"
                                                                    for="male_{{ $i }}">Male</label></div>
                                                            <div class="form-check"><input class="form-check-input"
                                                                    type="radio"
                                                                    name="devotees[{{ $i }}][gender]"
                                                                    id="female_{{ $i }}"
                                                                    value="female"><label class="form-check-label"
                                                                    for="female_{{ $i }}">Female</label>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-6"><label class="form-label">Age</label><input
                                                            type="number" name="devotees[{{ $i }}][age]"
                                                            class="form-control devotee-input" min="1"
                                                            max="120" data-index="{{ $i }}"
                                                            data-field="age" required></div>
                                                    <div class="col-md-6"><label
                                                            class="form-label">Email</label><input type="email"
                                                            name="devotees[{{ $i }}][email]"
                                                            class="form-control" required></div>
                                                    <div class="col-md-6"><label class="form-label">Mobile
                                                            Number</label><input type="tel"
                                                            name="devotees[{{ $i }}][phone_number]"
                                                            class="form-control" required></div>
                                                    <div class="col-md-4">
                                                        <label for="pincode-input-{{ $i }}"
                                                            class="form-label">Pincode</label>
                                                        <div class="pincode-wrapper">
                                                            <input type="text"
                                                                id="pincode-input-{{ $i }}"
                                                                name="devotees[{{ $i }}][pincode]"
                                                                class="form-control pincode-input"
                                                                data-index="{{ $i }}" maxlength="6"
                                                                pattern="\d{6}" required>
                                                            <div class="spinner-border spinner-border-sm text-secondary pincode-loader"
                                                                id="pincode-loader-{{ $i }}"></div>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-4"><label class="form-label">City</label><input
                                                            type="text" id="city-autofill-{{ $i }}"
                                                            name="devotees[{{ $i }}][city]"
                                                            class="form-control" readonly required></div>
                                                    <div class="col-md-4">
                                                        <label class="form-label">State</label>
                                                        <select id="state-autofill-{{ $i }}"
                                                            name="devotees[{{ $i }}][state]"
                                                    ...
                                                    <div class="col-12"><label class="form-label">Full Address</label>
                                                        <textarea name="devotees[{{ $i }}][address]" class="form-control" rows="2" required></textarea>
                                                    </div>
                                                    <div class="col-md-5"><label class="form-label">ID Proof
                                                            Type</label><select
                                                            name="devotees[{{ $i }}][id_type]"
                                                    ...
                                                    <div class="col-md-7"><label class="form-label">ID
                                                            Number</label><input type="text"
                                                            name="devotees[{{ $i }}][id_number]"
                                                            class="form-control" required></div>
                                                    <div class="col-12"><label class="form-label">Upload ID Proof
                                                            Photo</label><input class="form-control" type="file"
                                                            name="devotees[{{ $i }}][id_photo]"
                                                            accept="image/png, image/jpeg, image/jpg" required></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="form-check my-4"><input class="form-check-input" type="checkbox"
                                    value="" id="declarationCheck" required><label class="form-check-label"
                                    for="declarationCheck">Please Accept <a href="#" class="terms-link"
                                        data-bs-toggle="modal" data-bs-target="#termsModal">General Terms &
                                        Conditions</a> for the Advance Darshan Booking.</label></div>
            ...
            <div class="col-lg-4">
                <div class="card main-card summary-card">
                    ...
                    <div class="card-body p-4">
                        ...
                        <p class="text-muted"><i class="fas fa-map-marker-alt me-1"></i>
                            {{ $temple->location ?? 'N/A' }}
                        </p>
                        <ul class="list-unstyled summary-list">
                            ...
                            <li>
                                ...
                                <strong>
                                    {{ \Carbon\Carbon::parse($darshanSlot->start_time)->format('h:i A') }} -
                                    {{ \Carbon\Carbon::parse($darshanSlot->end_time)->format('h:i A') }}
                                </strong>
                            </li>
                        ...
                        <ul class="list-unstyled summary-list">
                            <li><span>Base Price</span> <span>₹{{ number_format($temple->darshan_charge ?? 0, 2) }} x
                                    {{ $bookingData['number_of_people'] }}</span></li>
                            <li><span>Sub Total</span>
                                <span>₹{{ number_format(($temple->darshan_charge ?? 0) * $bookingData['number_of_people'], 2) }}</span>
                            </li>
                        </ul>
                        <div class="d-flex justify-content-between align-items-center fw-bold mt-3 total-payable">
                            ...
                            <span
                                class="fs-5 text-success">₹{{ number_format(($temple->darshan_charge ?? 0) * $bookingData['number_of_people'], 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    @else
                        ...
                        <p><strong>Default Terms:</strong> Please contact the temple administration for booking terms
                            and conditions.</p>
                    @endif
                </div>
