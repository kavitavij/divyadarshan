            ...
            <div class="block md:hidden space-y-4">
                @foreach (session('cart') as $index => $item)
                    ...
                    <div class="border rounded-lg shadow p-4 bg-white">
                        ...
                        <div class="text-sm text-gray-500 mb-2">Type: {{ ucfirst($item['type']) }}</div>
                        ...
                        <div class="mt-2 text-sm">Price: <span
                                class="font-medium">₹{{ number_format($item['price'], 2) }}</span></div>
                        ...
                        <form action="{{ route('cart.remove', $index) }}" method="POST" class="mt-3">
                            @csrf
                            @method('DELETE')
                            <button class="w-full px-3 py-2 bg-red-500 text-white rounded">Remove</button>
                        </form>
                    </div>
                @endforeach
            </div>
