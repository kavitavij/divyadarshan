        ...
        <div class="flex flex-col md:flex-row justify-center gap-4 mb-8">
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('ebooks.index', ['type' => $selectedType]) }}"
                    class="px-3 py-1 rounded {{ !$selectedLanguage ? 'bg-blue-600 text-white' : 'bg-gray-200' }}">All</a>
                @foreach ($languages as $language)
                    <a href="{{ route('ebooks.index', ['language' => $language, 'type' => $selectedType]) }}"
                        class="px-3 py-1 rounded {{ $selectedLanguage == $language ? 'bg-blue-600 text-white' : 'bg-gray-200' }}">{{ $language }}</a>
                @endforeach
            </div>
            <div class="flex items-center gap-2">
                <span class="font-semibold">Type:</span>
                <a href="{{ route('ebooks.index', ['language' => $selectedLanguage]) }}"
                    class="px-3 py-1 rounded {{ !$selectedType ? 'bg-blue-600 text-white' : 'bg-gray-200' }}">All</a>
                <a href="{{ route('ebooks.index', ['type' => 'free', 'language' => $selectedLanguage]) }}"
                    class="px-3 py-1 rounded {{ $selectedType == 'free' ? 'bg-blue-600 text-white' : 'bg-gray-200' }}">Free</a>
                <a href="{{ route('ebooks.index', ['type' => 'paid', 'language' => $selectedLanguage]) }}"
                    class="px-3 py-1 rounded {{ $selectedType == 'paid' ? 'bg-blue-600 text-white' : 'bg-gray-200' }}">Paid</a>
            </div>
        </div>

        @if ($ebooks->isEmpty())
            <p class="text-center text-gray-500">No eBooks match your selected filters. Please try a different selection.
            </p>
        @else
                ...
                @foreach ($ebooks as $ebook)
                    <div class="card bg-white shadow-lg rounded-lg overflow-hidden flex flex-col">
                        <div class="relative">
                            ...
                            {{-- Paid/Free Badge --}}
                            <div
                                class="absolute top-2 right-2 px-2 py-1 rounded text-xs font-bold {{ $ebook->type == 'paid' ? 'bg-amber-500 text-white' : 'bg-green-600 text-white' }}">
                                {{ ucfirst($ebook->type) }}
                            </div>
                        </div>
                            ...
                            <div class="mt-4">
                                ...
                                @else
                                    <a href="{{ Storage::url($ebook->ebook_file_path) }}" target="_blank"
                                        class="inline-block bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 w-full text-center">
                                        Read PDF
                                    </a>
                                @endif
                            </div>
