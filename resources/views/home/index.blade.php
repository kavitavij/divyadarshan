...
@section('content')
    @if (session('success'))
        <div class="container mx-auto px-4">
            <div class="bg-green-100 border-green-400 text-green-700 dark:bg-green-900/50 dark:border-green-700 dark:text-green-300 px-4 py-3 rounded relative mb-6" role="alert">
     ...
     <section class="py-16 bg-gray-50 dark:bg-gradient-to-b dark:from-slate-900 dark:to-gray-900">
        <div class="container mx-auto px-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                @php
                    $modules = [
                        ['icon' => 'fa-gopuram', 'title' => 'Temple Info', 'desc' => 'Explore temple details, timings, and history.', 'url' => $firstTemple ? route('temples.show', $firstTemple->id) : route('temples.index')],
                        ['icon' => 'fa-calendar-check', 'title' => 'Book Darshan', 'desc' => 'Secure your spot with real-time slot booking.', 'url' => route('booking.index')],
                        ['icon' => 'fa-bed', 'title' => 'Accommodation', 'desc' => 'Find and book comfortable stays near temples.', 'url' => route('stays.index')],
                        ['icon' => 'fa-praying-hands', 'title' => 'Sevas & Poojas', 'desc' => 'Participate in sacred rituals from anywhere.', 'url' => route('sevas.booking.index')],
                        ['icon' => 'fa-hand-holding-heart', 'title' => 'Donations', 'desc' => 'Contribute to temples and receive blessings.', 'url' => route('donations.index')],
                        ['icon' => 'fa-book-open', 'title' => 'E-Books', 'desc' => 'Access a library of spiritual texts.', 'url' => route('ebooks.index')],
                        ['icon' => 'fa-headset', 'title' => 'Spiritual Guidance', 'desc' => 'Connect with experts for spiritual advice.', 'url' => route('info.contact')],
                        ['icon' => 'fa-star', 'title' => 'Guest Reviews', 'desc' => 'Read experiences from fellow devotees.', 'url' => route('reviews.index')],
                    ];
                @endphp
                @foreach ($modules as $module)
                    <a href="{{ $module['url'] }}" class="group block text-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-8 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:border-yellow-400 dark:hover:border-yellow-500">
                        ...
                        <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-100 transition-colors group-hover:text-yellow-600 dark:group-hover:text-yellow-400">{{ $module['title'] }}</h3>
                ...
                <div class="relative flex flex-col items-start space-y-8">
                    <div class="flow-step">
                        ...
                        <div>
                            ...
                            <p class="text-gray-500 dark:text-gray-400">Select a darshan, seva, or accommodation from our wide range of options.</p>
                        </div>
                    </div>
                    <div class="flow-step">
                        ...
                        <div>
                            ...
                            <p class="text-gray-500 dark:text-gray-400">Enter the required information for all devotees and complete the booking.</p>
                        </div>
                    </div>
                    <div class="flow-step">
                        ...
                        <div>
                            ...
                            <p class="text-gray-500 dark:text-gray-400">Pay securely through our trusted payment gateway to confirm your service.</p>
                        </div>
                    </div>
                    <div class="flow-step">
                        ...
                        <div>
                            ...
                            <p class="text-gray-500 dark:text-gray-400">Get instant confirmation and receipts on WhatsApp and your email.</p>
                        </div>
                    </div>
                </div>
            </div>
...
.temple-nav-next, .temple-nav-prev {
    color: #fff;
...
.temple-nav-next:hover, .temple-nav-prev:hover {
    background-color: rgba(0,0,0,0.5);
}
.temple-nav-next::after, .temple-nav-prev::after {
    font-size: 20px;
    font-weight: bold;
}
...
.guidance-overlay { position: absolute; top:0; left:0; width:100%; height:100%; background: rgba(0,0,0,0.65); }
.guidance-content { position: relative; z-index:2; max-width:650px; margin:0 auto; }
.guidance-button { background: linear-gradient(135deg, #facc15, #f59e0b); color:#000; padding:14px 32px; font-size:1rem; font-weight:600; border-radius:30px; text-decoration:none; box-shadow:0 4px 15px rgba(250,204,21,0.35); transition: all 0.3s ease-in-out; display: inline-block; }
.guidance-button:hover { transform: scale(1.05); box-shadow:0 6px 20px rgba(250,204,21,0.45); }
