...
<style>
    ...
    .notification-item.unread {
        background-color: #fefce8; /* A light yellow to match your theme */
        font-weight: 500;
    }
    .notification-icon {
        font-size: 1.5rem;
        margin-right: 1rem;
        color: #ca8a04; /* A darker yellow */
    }
    .notification-time {
        font-size: 0.85rem;
        color: #6c757d;
...
<div class="container-fluid">
    <div class="card shadow-sm">
        <div class="card-header bg-light">
            ...
            <form action="{{ route('temple-manager.notifications.all') }}" method="GET" class="d-flex align-items-center">
                <input type="date" name="date" value="{{ request('date') }}" class="form-control form-control-sm me-2" />
                <button type="submit" class="btn btn-sm btn-warning">
                    <i class="fas fa-filter me-1"></i> Filter
                </button>
                @if(request('date'))
                    <a href="{{ route('temple-manager.notifications.all') }}" class="btn btn-sm btn-outline-secondary ms-2">Clear</a>
                @endif
            </form>
        </div>
